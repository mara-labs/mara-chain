...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
ETHERMINT_BINARY = marad
ETHERMINT_DIR = mara-chain
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/mara-labs/mara-chain.git
NAMESPACE := mara-labs
PROJECT_NAME = mara-chain
DOCKER_IMAGE := $(NAMESPACE)/$(PROJECT_NAME)
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=mara \
...
#Print Build flags when needed
#$(info $$BUILD_FLAGS -> [$(BUILD_FLAGS)])
#$(info )
#$(info $$ldflags -> [$(ldflags)])
#$(info )
#$(info $$COSMOS_BUILD_OPTIONS -> [$(COSMOS_BUILD_OPTIONS)])
#$(info )
...
docker-build:
	...
	docker create --name mara -t -i ${DOCKER_IMAGE}:latest mara
	# move the binaries to the ./build directory
	mkdir -p ./build/
	docker cp mara:/usr/bin/marad ./build/
...
PACKAGE_NAME:=github.com/mara-labs/mara-chain
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/mara-labs/mara-chain/types"
	godoc -http=:6060
...
test-import:
	go test -run TestImporterTestSuite -v --vet=off github.com/mara-labs/mara-chain/tests/importer
...
ifeq ($(OS),Windows_NT)
	...
	IF not exist "build/node0/$(ETHERMINT_BINARY)/config/genesis.json" docker run --rm -v $(CURDIR)/build\ethermint\Z marad/node "./marad testnet --v 4 -o /ethermint --keyring-backend=test --ip-addresses maradnode0,maradnode1,maradnode2,maradnode3"
	docker-compose up -d
else
	mkdir -p localnet-setup
	@$(MAKE) localnet-build

	if ! [ -f localnet-setup/node0/$(ETHERMINT_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/localnet-setup:/ethermint:Z marad/node "./marad testnet --v 4 -o /ethermint --keyring-backend=test --ip-addresses maradnode0,maradnode1,maradnode2,maradnode3"; fi
	docker-compose up -d
endif
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\marad:mara\Z marad/node "./marad unsafe-reset-all --home=/mara"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\marad:mara\Z marad/node "./marad unsafe-reset-all --home=/mara"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\marad:mara\Z marad/node "./marad unsafe-reset-all --home=/mara"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\marad:mara\Z marad/node "./marad unsafe-reset-all --home=/mara"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/marad:/mara:Z marad/node "./marad unsafe-reset-all --home=/mara"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/marad:/mara:Z marad/node "./marad unsafe-reset-all --home=/mara"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/marad:/mara:Z marad/node "./marad unsafe-reset-all --home=/mara"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/marad:/mara:Z marad/node "./marad unsafe-reset-all --home=/mara"
endif

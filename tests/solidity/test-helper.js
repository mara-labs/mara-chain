...
function checkTestEnv() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network mara', 'run all tests using mara network')
    .example('$0 --network mara --allowTests=test1,test2', 'run only test1 and test2 using mara network')
    .help('h').alias('h', 'help')
    .describe('network', 'set which network to use: ganache|mara')
    ...
    .boolean('verbose-log').describe('verbose-log', 'print marad output, default false')
    .argv;
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'mara' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or mara');
    }
    else {
...
function loadTests(runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach(dirname => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-mara'];
...
function performTestSuite({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-mara';
...
function setupNetwork({ runConfig, timeout }) {
  if (runConfig.network !== 'mara') {
    // no need to start ganache. Truffle will start it
    return;
  }

  // Spawn the mara process

  const spawnPromise = new Promise((resolve, reject) => {
    const maradProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', runConfig.verboseLog ? 'pipe' : 'ignore', 'pipe'],
    });

    logger.info(`Starting marad process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      maradProc.stdout.pipe(process.stdout);
    }
    maradProc.stderr.on('data', d => {
      const oLine = d.toString();
      if (runConfig.verboseLog) {
        process.stdout.write(oLine);
      }
      if (oLine.indexOf('Starting JSON-RPC server') !== -1) {
        logger.info('marad started');
        resolve(maradProc);
      }
    });
  });
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start marad timeout!')), timeout);
  });
